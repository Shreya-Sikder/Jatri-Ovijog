...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_GET['id'])) {
    $id = intval($_GET['id']);
    $sql = "UPDATE reports SET downvotes = downvotes + 1 WHERE id = $id";
    if ($conn->query($sql) === TRUE) {
        echo json_encode(["success" => "Downvote successful"]);
    } else {
        http_response_code(500);
        echo json_encode(["error" => "Failed to downvote: " . $conn->error]);
    }
} else {
